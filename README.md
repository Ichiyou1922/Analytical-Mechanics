...
- なぜT_END=1.42877?
  - $y(t)=H-\frac{1}{2}gt$ でH=10としたとき，地面に到達する時間は大体これくらいの時間．
...
1. 速度vの近似
  - 区間の平均速度は $v_{i} \neq \frac{v_{i+1}-v_{i}}{dt}$ 
  - 運動エネルギー= $\frac{1}{2]m(\frac{v_{i+1}-v_{i}}{dt})^2$
2. 高さhの平均をとる $h_{mean} = \frac{y_{i+1}+y_{i}}{2}$
  - ポテンシャル = $mg(\frac{y_{i+1}+y_{i}}{2})$
3. 部分作用 $S_{i}$
...
- 左辺の  $m\frac{y_{i+1}-2y_{i}+y_{i-1}}{(\delta t)^2}$ は加速度の離散近似になっている．
